  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-4 gap-2">
        {stats.map((s, i) => (
          ...
          >
            ...
            <span
              className={`text-[8px] font-medium ${s.up ? "text-green-400" : "text-accent"}`}
            >
              {s.change}
            </span>
          </motion.div>
      ...
      <div className="rounded-lg bg-dark-card p-3">
        <span className="text-[9px] text-on-dark-muted mb-2 block">
          Pedidos Recentes
        </span>
        {recentOrders.map((o, i) => (
          ...
          >
            <div className="flex items-center gap-3">
              <span className="text-[9px] font-mono text-on-dark-muted">
                {o.id}
              </span>
              <span className="text-[9px] text-on-dark">{o.client}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-[9px] font-semibold text-on-dark">
                {o.value}
              </span>
              <span
                className={`text-[8px] px-1.5 py-0.5 rounded font-medium ${o.color}`}
              >
                {o.status}
              </span>
            </div>
...
function ClientsContent() {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between mb-1">
        <span className="text-[10px] font-semibold text-on-dark">
          Clientes Cadastrados
        </span>
        <span className="text-[8px] text-primary cursor-pointer">
          + Novo cliente
        </span>
      </div>
      <div className="rounded-lg bg-dark-card overflow-hidden">
        <div className="grid grid-cols-5 gap-2 px-3 py-2 border-b border-dark-border/50">
          {["Nome", "Email", "Telefone", "Score", "Status"].map((h) => (
            <span
              key={h}
              className="text-[8px] font-semibold text-on-dark-muted uppercase"
            >
              {h}
            </span>
          ))}
          ...
          >
            ...
            <div className="flex items-center gap-1">
              <Star
                size={8}
                className={
                  c.score > 70 ? "text-accent" : "text-on-dark-muted/50"
                }
              />
              <span
                className={`text-[9px] font-semibold ${c.score > 70 ? "text-on-dark" : "text-on-dark-muted"}`}
              >
                {c.score}
              </span>
            </div>
            <span
              className={`text-[8px] px-1.5 py-0.5 rounded font-medium self-center w-fit ${c.status === "Ativo" ? "bg-green-500/20 text-green-400" : "bg-on-dark-muted/10 text-on-dark-muted"}`}
            >
              {c.status}
            </span>
          </motion.div>
          ...
          <div className="flex gap-1 mt-1.5">
            {["bg-primary", "bg-accent", "bg-green-400"].map((c, i) => (
              <div key={i} className="flex items-center gap-0.5">
                ...
                <span className="text-[7px] text-on-dark-muted">
                  {["Vendas", "Serv.", "Outros"][i]}
                </span>
              </div>
            ))}
          </div>
...
function AutomationContent() {
  ...
  return (
    <div className="space-y-3">
      <div className="rounded-lg bg-dark-card p-3">
        <span className="text-[9px] text-on-dark-muted mb-3 block">
          Fluxo de Automação
        </span>
        <div className="flex items-center justify-between px-2">
          {steps.map((step, i) => (
            ...
            >
              <div
                className={`w-8 h-8 rounded-lg flex items-center justify-center ${i === 0 ? "bg-primary/20" : i === 1 ? "bg-accent/20" : i === 2 ? "bg-green-500/20" : "bg-primary/10"}`}
              >
                {i === 0 ? (
              ...
              <div>
                <span className="text-[9px] font-medium text-on-dark block">
                  {step}
                </span>
              </div>
              {i < steps.length - 1 && (
                <ArrowRight size={10} className="text-on-dark-muted/30 ml-1" />
              )}
            </motion.div>
          ))}
        </div>
      ...
      <div className="rounded-lg bg-dark-card p-3">
        ...
        {flows.map((f, i) => (
          ...
          >
            ...
            <div className="flex items-center gap-2">
              <span className="text-[8px] text-on-dark-muted">
                {f.steps} etapas
              </span>
              <span
                className={`text-[8px] px-1.5 py-0.5 rounded font-medium ${f.status === "Ativo" ? "bg-green-500/20 text-green-400" : "bg-on-dark-muted/10 text-on-dark-muted"}`}
              >
                {f.status}
              </span>
            </div>
    ...
    <section className="py-24 lg:py-32 bg-dark-base relative overflow-hidden">
      ...
      <div ref={ref} className="relative max-w-7xl mx-auto px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-12">
          ...
          <TypeWriter
            text="Imagine o sistema ideal para você"
            ...
            <div className="flex">
              <div className="w-12 border-r border-dark-border py-3 flex flex-col items-center gap-3 shrink-0">
                {sidebarIcons.map((Icon, i) => (
                  <div
                    key={i}
                    className={`w-7 h-7 rounded-lg flex items-center justify-center ${i === 0 ? "bg-primary/20" : ""}`}
                  >
                    <Icon
                      size={13}
                      className={
                        i === 0 ? "text-primary" : "text-on-dark-muted/50"
                      }
                    />
                  </div>
                ))}
