...
import {
  ArrowRight, Home, Users, ShoppingCart, BarChart3, Settings,
      ...
      <div className="flex">
        <div className="w-12 border-r border-dark-border py-3 flex flex-col items-center gap-1 shrink-0">
          {sidebarItems.map((item, i) => (
            <button key={i} onClick={() => setActiveSidebar(i)} className="group relative">
              <div className={`w-7 h-7 rounded-lg flex items-center justify-center transition-all duration-300 ${activeSidebar === i ? 'bg-primary/20 scale-110' : 'hover:bg-dark-card hover:scale-105'}`}>
                <item.icon size={13} className={`transition-colors duration-300 ${activeSidebar === i ? 'text-primary' : 'text-on-dark-muted/50 group-hover:text-on-dark-muted'}`} />
              </div>
          ...
          <div className="grid grid-cols-4 gap-2">
            {stats.map((stat, i) => (
              <motion.div
                ...
                className={`rounded-lg bg-dark-card p-2 cursor-pointer transition-all duration-300 ${hoveredStat === i ? 'bg-dark-border/50 scale-105 shadow-lg shadow-primary/10' : ''}`}
              >
                ...
                <p className={`text-xs font-bold transition-colors duration-300 ${hoveredStat === i ? 'text-primary' : 'text-on-dark'}`}>{stat.value}</p>
                <span className={`text-[7px] font-medium ${stat.change.startsWith('+') ? 'text-green-400' : 'text-accent'}`}>{stat.change}</span>
              </motion.div>
            ))}
          </div>
                ...
                {bars.map((h, i) => (
                  <div key={i} className="flex-1 relative group h-full flex items-end"
                    ...
                    <motion.div
                      ...
                      className={`w-full rounded-sm bg-gradient-to-t from-primary to-accent cursor-pointer transition-all duration-200 ${hoveredBar === i ? 'opacity-100 scale-x-110' : 'opacity-75 hover:opacity-90'}`}
                    />
                  </div>
                ))}
              </div>
          ...
          <div className="rounded-lg bg-dark-card p-2">
            ...
            {transactions.map((row, i) => (
              <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
                ...
                className={`flex items-center justify-between py-1 border-b border-dark-border/50 last:border-0 cursor-pointer rounded px-1 transition-all duration-200 ${hoveredRow === i ? 'bg-dark-border/30 scale-[1.02]' : ''}`}
              >
                <span className={`text-[8px] transition-colors duration-200 ${hoveredRow === i ? 'text-on-dark' : 'text-on-dark-muted'}`}>
                  {row.name} – {row.amount}
                </span>
                <span className={`text-[7px] px-1.5 py-0.5 rounded font-medium transition-all duration-200 ${row.color} ${hoveredRow === i ? 'scale-110' : ''}`}>
                  {row.status}
                </span>
              </motion.div>
            ))}
