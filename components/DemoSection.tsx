...
const chartBars = [
  { label: 'Jan', h: 45 }, { label: 'Fev', h: 60 }, { label: 'Mar', h: 35 },
  { label: 'Abr', h: 75 }, { label: 'Mai', h: 55 }, { label: 'Jun', h: 90 },
  { label: 'Jul', h: 70 },
];
    ...
    <section className="py-24 lg:py-32 bg-dark-base relative overflow-hidden">
      <div ref={ref} className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 40 }} animate={isVisible ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.2, duration: 0.7 }}>
            <DashboardCard>
              <div className="space-y-3">
                ...
                <div className="flex gap-1.5 mb-2">
                  {['Diário', 'Mensal', 'Anual'].map((t, i) => (
                    <span key={t} className={`text-[8px] px-2 py-1 rounded-md font-medium ${i === 1 ? 'bg-primary/20 text-primary' : 'bg-dark-card text-on-dark-muted'}`}>{t}</span>
                  ))}
                </div>
                <div className="flex items-end gap-1.5 h-28">
                  {chartBars.map((bar, i) => (
                    <div key={bar.label} className="flex-1 flex flex-col items-center gap-1 h-full justify-end"
                      ...
                      {hoveredBar === i && (
                        <span className="text-[7px] font-bold text-primary">R$ {(bar.h * 320).toLocaleString('pt-BR')}</span>
                      )}
                      <motion.div
                        initial={{ height: 0 }}
                        animate={isVisible ? { height: `${(bar.h / 100) * 96}px` } : {}}
                        transition={{ delay: 0.4 + i * 0.08, duration: 0.6, ease: 'easeOut' }}
                        className={`w-full rounded-md bg-gradient-to-t from-primary to-accent transition-all duration-200 cursor-pointer ${hoveredBar === i ? 'opacity-100 scale-x-110' : 'opacity-75'}`}
                      />
          ...
          <motion.div initial={{ opacity: 0, y: 40 }} animate={isVisible ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.35, duration: 0.7 }}>
            <DashboardCard>
              <div className="space-y-2">
                ...
                {orders.map((order, i) => (
                  <motion.div key={order.id} initial={{ opacity: 0, x: 20 }} animate={isVisible ? { opacity: 1, x: 0 } : {}}
                    transition={{ delay: 0.5 + i * 0.1 }}
                    onMouseEnter={() => setHoveredRow(i)} onMouseLeave={() => setHoveredRow(null)}
                    className={`flex items-center justify-between py-2 px-2.5 rounded-lg bg-dark-card cursor-pointer transition-all duration-200 ${hoveredRow === i ? 'bg-dark-border/40 scale-[1.02]' : ''}`}
                  >
                    <span className={`text-[10px] transition-colors ${hoveredRow === i ? 'text-on-dark' : 'text-on-dark-muted'}`}>{order.id}</span>
                    <span className={`text-[8px] px-2 py-0.5 rounded-md font-medium ${order.color} text-primary-foreground`}>{order.status}</span>
                  </motion.div>
                ))}
              </div>
            </DashboardCard>
