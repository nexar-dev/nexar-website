...
export default function NeedsQuestionnaire({ onClose }: { onClose: () => void }) {
  ...
  const renderStep = () => {
    switch (step) {
      case 0:
        return (<div><SectionTitle>Dados Iniciais</SectionTitle><SectionDesc>Informações básicas para contato.</SectionDesc>
      ...
      case 1:
        return (<div><SectionTitle>Tipo de Negócio</SectionTitle><SectionDesc>Qual é o segmento da sua empresa?</SectionDesc>
          <ChipSelect options={BUSINESS_TYPES} selected={data.business_type} onToggle={(v) => update("business_type", v)} /></div>);
      case 2:
        return (<div><SectionTitle>Objetivos do Sistema</SectionTitle><SectionDesc>O que você espera que o sistema resolva? Selecione quantos quiser.</SectionDesc>
          <ChipSelect options={OBJECTIVES} selected={data.objectives} onToggle={(v) => toggleArray("objectives", v)} multi /></div>);
      case 3:
        return (<div><SectionTitle>Funcionalidades Desejadas</SectionTitle><SectionDesc>Quais funcionalidades seriam ideais?</SectionDesc>
          <ChipSelect options={FEATURES} selected={data.features} onToggle={(v) => toggleArray("features", v)} multi /></div>);
      case 4:
        return (<div><SectionTitle>Uso e Dispositivos</SectionTitle><SectionDesc>Quem vai usar e em quais dispositivos?</SectionDesc>
      ...
      case 5:
        return (<div><SectionTitle>Login e Relatórios</SectionTitle><SectionDesc>Configurações de acesso e análise de dados.</SectionDesc>
          <div className="space-y-6">
      ...
      case 6:
        return (<div><SectionTitle>Integrações e Prioridade</SectionTitle><SectionDesc>Últimas informações técnicas.</SectionDesc>
          <div className="space-y-6">
      ...
      case 7:
        return (<div><SectionTitle>Resumo Final</SectionTitle><SectionDesc>Descreva livremente o que imagina para o sistema.</SectionDesc>
  ...
  return (
    <div className="fixed inset-0 z-50 bg-dark-base flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-3xl mx-auto px-6 py-8">
          <AnimatePresence mode="wait">
            <motion.div key={step} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.3, ease: [0.2, 0, 0, 1] }}>
              {renderStep()}
            </motion.div>
          </AnimatePresence>
        </div>
      ...
      <div className="border-t border-dark">
        <div className="max-w-3xl mx-auto px-6 py-4 flex items-center justify-between">
          <button onClick={() => setStep((s) => s - 1)} disabled={step === 0} className="inline-flex items-center gap-2 rounded-xl px-5 py-2.5 text-sm font-medium text-on-dark-muted hover:text-on-dark disabled:opacity-30 transition-all">
            <ArrowLeft size={16} /> Voltar
          </button>
          {step < STEPS.length - 1 ? (
            <button onClick={handleNext} disabled={!canNext()} className="inline-flex items-center gap-2 rounded-xl bg-gradient-purple px-6 py-2.5 text-sm font-semibold text-primary-foreground shadow-glow transition-all hover:scale-105 hover:opacity-90 active:scale-110 disabled:opacity-40">
              Próximo <ArrowRight size={16} />
            </button>
          ) : (
            <button onClick={handleSubmit} disabled={submitting} className="inline-flex items-center gap-2 rounded-xl bg-gradient-purple px-6 py-2.5 text-sm font-semibold text-primary-foreground shadow-glow transition-all hover:scale-105 hover:opacity-90 active:scale-110 disabled:opacity-40">
