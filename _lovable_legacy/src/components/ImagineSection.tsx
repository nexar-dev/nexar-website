      ...
      <div className="rounded-lg bg-dark-card overflow-hidden">
        ...
        {clients.map((c, i) => (
          <motion.div key={c.name} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }}
            ...
            <span className={`text-[8px] px-1.5 py-0.5 rounded font-medium self-center w-fit ${
              c.status === "Ativo" ? "bg-green-500/20 text-green-400" : "bg-on-dark-muted/10 text-on-dark-muted"
            }`}>{c.status}</span>
          </motion.div>
        ))}
      </div>
          ...
          {steps.map((step, i) => (
            <motion.div key={step} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.15 }}
              ...
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                i === 0 ? "bg-primary/20" : i === 1 ? "bg-accent/20" : i === 2 ? "bg-green-500/20" : "bg-primary/10"
              }`}>
              ...
              <div>
                <span className="text-[9px] font-medium text-on-dark block">{step}</span>
              </div>
          ...
          <motion.div key={f.name} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 + i * 0.1 }}
            ...
            <div className="flex items-center gap-2">
              ...
              <span className={`text-[8px] px-1.5 py-0.5 rounded font-medium ${
                f.status === "Ativo" ? "bg-green-500/20 text-green-400" : "bg-on-dark-muted/10 text-on-dark-muted"
              }`}>{f.status}</span>
            </div>
          </motion.div>
        ))}
